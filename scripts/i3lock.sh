#!/usr/bin/env bash
# by fbt2lurker
# at https://www.reddit.com/r/unixporn/comments/3358vu/i3lock_unixpornworthy_lock_screen/cqkmxxi/
cfg_tmpdir="/run/user/$UID/i3lock-extra"
cfg_pixelize_scale='20'
...
main() {
	...
	if (( f_secloop )); then
		until i3lock -d -n -t -i "$image_file"; do
			true
		done
	else
		i3lock -d -n -t -i "$image_file"
	fi
}
